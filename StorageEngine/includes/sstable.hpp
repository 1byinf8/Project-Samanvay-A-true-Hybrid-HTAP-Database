...
public:
  BloomFilter(size_t expectedElements, double falsePositiveRate = 0.01) {
    size = static_cast<size_t>(-expectedElements * log(falsePositiveRate) /
                               (log(2) * log(2)));
...
public:
  ...
  bool create(const std::vector<std::pair<std::string, skiplist::Entry>>
                  &sortedEntries) {
    ...
    file.write(reinterpret_cast<const char *>(&header.minKeyLen),
               sizeof(header.minKeyLen));
    file.write(reinterpret_cast<const char *>(&header.maxKeyLen),
               sizeof(header.maxKeyLen));

    std::streampos dataStartOffset = file.tellp();
    dataStartOffset +=
        static_cast<std::streampos>(minKey.size() + maxKey.size());
    ...
    for (size_t i = 0; i < sortedEntries.size(); ++i) {
      ...
      if (i % SPARSE_INDEX_INTERVAL == 0) {
        sparseIndex.emplace_back(key, static_cast<uint64_t>(entryOffset),
      ...
      file.write(reinterpret_cast<const char *>(&entry.isDeleted),
      ...
      if (!sparseIndex.empty() && i % SPARSE_INDEX_INTERVAL == 0) {
        ...
        sparseIndex.back().size =
            static_cast<uint32_t>(currentPos - entryOffset);
      }
    }
    ...
    for (const auto &indexEntry : sparseIndex) {
      ...
      file.write(reinterpret_cast<const char *>(&indexEntry.offset),
                 sizeof(indexEntry.offset));
      file.write(reinterpret_cast<const char *>(&indexEntry.size),
                 sizeof(indexEntry.size));
    }
  ...
  std::optional<skiplist::Entry> get(const std::string &key) {
    ...
    auto it =
        std::upper_bound(sparseIndex.begin(), sparseIndex.end(), key,
  ...
  // Load SSTable metadata from existing file
  bool load() {
    std::ifstream file(filePath, std::ios::binary);
    if (!file.is_open()) {
      return false;
    ...
    minSequence = header.minSequence;
    maxSequence = header.maxSequence;
    entryCount = header.entryCount;

    // Read min/max key lengths (duplicated after header)
    ...
    // Read min/max keys
    minKey.resize(minKeyLen);
    file.read(&minKey[0], minKeyLen);
    maxKey.resize(maxKeyLen);
    file.read(&maxKey[0], maxKeyLen);
    ...
    for (uint32_t i = 0; i < header.indexCount; ++i) {
      ...
      sparseIndex.emplace_back(key, offset, size);
    }

    // Read bloom filter
    file.seekg(header.bloomFilterOffset);
    size_t bloomSize, numHashFunctions;
    file.read(reinterpret_cast<char *>(&bloomSize), sizeof(bloomSize));
    file.read(reinterpret_cast<char *>(&numHashFunctions),
              sizeof(numHashFunctions));

    // Recreate bloom filter (simplified - in production would deserialize
    // properly)
    bloomFilter =
        std::make_unique<BloomFilter>(entryCount > 0 ? entryCount : 1000);

    file.close();
    return true;
  }

  // Get the data start offset (after header and keys)
  uint64_t getDataStartOffset() const {
    // Header size + 2 * sizeof(uint32_t) for key lengths + key sizes
    return sizeof(SSTableHeader) + 2 * sizeof(uint32_t) + minKey.size() +
           maxKey.size();
  }

  // Range query support - returns all entries in [startKey, endKey] range
  std::vector<std::pair<std::string, skiplist::Entry>>
  rangeQuery(const std::string &startKey, const std::string &endKey) {

    std::vector<std::pair<std::string, skiplist::Entry>> results;

    // Quick boundary check
    if (startKey > maxKey || endKey < minKey) {
      return results;
    }
    ...
    // Read header to get index offset (used as data end boundary)
    SSTableHeader header;
    file.read(reinterpret_cast<char *>(&header), sizeof(header));
    uint64_t dataEndOffset = header.indexOffset;

    // Find the starting position using sparse index
    uint64_t seekOffset = getDataStartOffset(); // Default to start of data

    if (!sparseIndex.empty()) {
      auto it =
          std::upper_bound(sparseIndex.begin(), sparseIndex.end(), startKey,
                           [](const std::string &key, const IndexEntry &entry) {
                             return key < entry.key;
                           });

      if (it != sparseIndex.begin()) {
        --it; // Go to the entry before or equal to startKey
      }
      seekOffset = it->offset;
    }

    file.seekg(seekOffset);

    // Scan entries until we exceed endKey or reach end of data section
    while (file.good() && static_cast<uint64_t>(file.tellg()) < dataEndOffset) {
      uint32_t keyLen;
      if (!file.read(reinterpret_cast<char *>(&keyLen), sizeof(keyLen))) {
        break;
      }
      ...
      // Read sequence and deleted flag
      uint64_t seq;
      bool isDeleted;
      file.read(reinterpret_cast<char *>(&seq), sizeof(seq));
      file.read(reinterpret_cast<char *>(&isDeleted), sizeof(isDeleted));

      // Add to results if in range (including startKey)
      if (entryKey >= startKey) {
        results.emplace_back(entryKey,
                             skiplist::Entry(entryKey, value, seq, isDeleted));
      }
    }
  ...
  std::vector<std::pair<std::string, skiplist::Entry>> scanAll() {
    return rangeQuery(minKey, maxKey);
  }

  // Get sparse index for external use (e.g., by compaction)
  const std::vector<IndexEntry> &getSparseIndex() const { return sparseIndex; }
};
