...
void assertFail(const sql::ResultSet &rs, const std::string &name,
  ...
  if (!rs.success) {
    if (substr.empty() || rs.errorMessage.find(substr) != std::string::npos)
      pass(name);
    else
      fail(name,
           "Expected error with '" + substr + "' but got: " + rs.errorMessage);
  } else {
...
void assertRowCount(const sql::ResultSet &rs, size_t n,
  ...
  if (rs.rows.size() == n)
    ...
    fail(name, "Expected " + std::to_string(n) + " rows, got " +
...
void assertCell(const sql::ResultSet &rs, size_t row, size_t col,
  ...
  if (rs.rows[row][col] == expected)
    ...
    fail(name, "Expected '" + expected + "' got '" + rs.rows[row][col] + "'");
}
...
struct Ctx {
  ...
  sql::ResultSet run(const std::string &sql) { return executor->execute(sql); }

  std::string explain(const std::string &sql) { return executor->explain(sql); }
};
...
void testCreateTable() {
  ...
  auto r = ctx.run("CREATE TABLE users ("
                   "id BIGINT NOT NULL, "
                   "name VARCHAR(255), "
                   "age INT, "
                   "score DOUBLE, "
  ...
  check(schema->primaryKeyColumn == "id", "First NOT NULL col becomes PK");
  ...
  auto r2 = ctx.run("CREATE TABLE users (id BIGINT NOT NULL, x INT)");
  assertFail(r2, "Duplicate CREATE TABLE fails", "already exists");

  // All supported types
  auto r3 = ctx.run("CREATE TABLE types_test ("
                    "a BIGINT NOT NULL, "
                    "b INT, "
                    "c DOUBLE, "
                    "d FLOAT, "
  ...
  auto r4 =
      ctx.run("CREATE TABLE Orders (order_id BIGINT NOT NULL, amount DOUBLE)");
  assertOk(r4, "CREATE TABLE with mixed-case name");
  check(ctx.registry->tableExists("orders"), "Stored as lowercase in registry");

  // Implicit PK -- first column when no NOT NULL specified
  auto r5 = ctx.run("CREATE TABLE implicit_pk (id BIGINT, val VARCHAR(100))");
...
void testDropTable() {
  ...
  assertFail(ctx.run("DROP TABLE nonexistent"), "DROP TABLE nonexistent fails",
             "does not exist");

  // Drop and recreate
  ctx.run("CREATE TABLE phoenix (id BIGINT NOT NULL)");
  ctx.run("DROP TABLE phoenix");
  auto r3 = ctx.run("CREATE TABLE phoenix (id BIGINT NOT NULL, extra INT)");
...
void testInsert() {
  ...
  auto r = ctx.run("INSERT INTO users (id, name, age, salary) "
  ...
  auto r2 = ctx.run("INSERT INTO users VALUES (2, 'Bob', 25, 72000.00)");
  ...
  assertFail(ctx.run("INSERT INTO users (id, name) VALUES (99, 'T', 'extra')"),
...
void testSelectFullScan() {
  ...
  check(plan.find("FULL_SCAN") != std::string::npos, "EXPLAIN shows FULL_SCAN");
}
...
void testSelectWhere() {
  ...
  // OR
  auto r6 =
      ctx.run("SELECT * FROM orders WHERE region = 'APAC' OR region = 'EMEA'");
  ...
  std::string plan = ctx.explain("SELECT * FROM orders WHERE amount > 100");
...
void testUpdate() {
  ...
  assertFail(ctx.run("UPDATE orders SET fake_col = 'x' WHERE id = 1"),
             "UPDATE unknown column rejected", "Unknown column");

  // UPDATE wrong type rejected
  assertFail(ctx.run("UPDATE orders SET amount = 'notanumber' WHERE id = 1"),
...
void testDelete() {
  ...
  // Verify row exists
  assertRowCount(ctx.run("SELECT * FROM users WHERE id = 99"), 1,
                 "Row 99 exists before delete");
  ...
  assertFail(ctx.run("DELETE FROM users"), "DELETE without WHERE rejected",
             "WHERE");
...
void testSchemaPersistence() {
  ...
  // Read it back in a fresh instance
  {
    ...
    check(reg2.tableExists("persist_test"), "Schema survives reload");
    auto s = reg2.getSchema("persist_test");
    check(s.has_value() && s->columnCount() == 2,
          "Reloaded schema has 2 columns");
    check(s.has_value() && s->primaryKeyColumn == "id", "PK column preserved");
  }
...
void testRowCodecIntegrity() {
  ...
  sql::Row row = {
      {"order_id", "42"}, {"customer", "TestUser"}, {"amount", "999.99"}};
  ...
  check(val.has_value() && *val == "999.99", "getColumnValue extracts amount");
...
void testEdgeCases() {
  ...
  assertFail(ctx.run("SELECT * FROM ghost"), "SELECT from non-existent table",
             "does not exist");
  ...
  assertRowCount(ctx.run("SELECT * FROM users WHERE id = 50"), 0,
                 "Row gone after delete");
  ...
  std::string plan = ctx.explain("INSERT INTO users VALUES (1,'x',1)");
...
void testHTAPMixedWorkload() {
  ...
  // Insert 10 rows
  for (int i = 1; i <= 10; i++) {
    ctx.run("INSERT INTO orders VALUES (" + std::to_string(i) + ", 'Customer" +
            std::to_string(i) + "', " + std::to_string(i * 50.0) +
            ", 'pending')");
  }
  ...
  // OLTP update
  auto r2 = ctx.run("UPDATE orders SET status = 'shipped' WHERE id = 3");
  ...
  assertRowCount(ctx.run("SELECT * FROM orders WHERE id = 10"), 0,
