...
namespace sql
{
    ...
    public:
        ...
        {
            // Grab direct pointer to router from the engine
            // This is the SINGLE decision maker for all query routing
        ...
        // ── EXPLAIN — shows the router's query plan without executing ─
        std::string explain(const std::string &sql)
        {
            ...
            // Build the exact same QueryRequest that executeSelect would build
            query::QueryRequest request = buildQueryRequest(select, *schema);

            // Let the router plan it and return the human-readable explanation
    ...
    private:
        ...
        // Translates what SQL wants into what the router understands
        // ================================================================
            ...
            const columnar::TableSchema &schema) const
        {
            ...
            // Inspect selectList for aggregation functions
            bool hasAgg = false;
            ...
            // ── AGGREGATION: SELECT SUM/COUNT/AVG/MIN/MAX(col) ───────
            // Router will route this to columnar path (Level 4+)
            if (hasAgg)
            {
            ...
            // ── POINT LOOKUP: SELECT ... WHERE pk = value ─────────────
            // Router will use row path only (memtable + L0-L3)
            if (stmt->whereClause)
            {
                ...
                if (pkVal.has_value())
                {
                    std::string key = SchemaRegistry::encodeKey(tableName,
                                                                *pkVal);
                ...
                // Router estimates row count and decides:
                //   < 10K rows  → row path only
                //   >= 10K rows → row + columnar paths
                auto req = query::QueryRequest::rangeScan(prefix, endKey);
                req.selectColumns = selectedCols;
                return req;
            }

            // ── FULL SCAN: SELECT * FROM table (no WHERE) ─────────────
            // Router uses both row + columnar paths
        ...
        // Router already decided what to do — we just carry it out
        // ================================================================
            ...
            const hsql::SelectStatement *stmt) const
        {
            ...
            // Collect selected columns for result formatting
            std::vector<std::string> selectedCols;
            for (const auto *expr : *stmt->selectList)
            {
            ...
            // ── POINT LOOKUP ─────────────────────────────────────────
            // plan: useRowPath=true, useColumnarPath=false
            // Hits memtable first, then row SSTables L0-L3
            if (plan.type == query::QueryType::POINT_LOOKUP)
            {
                ...
                return ResultFormatter::fromSingleRow(schema, *result,
                                                      selectedCols);
            }

            // ── AGGREGATION ──────────────────────────────────────────
            // plan: useColumnarPath=true, useRowPath=false
            // Hits columnar files (Level 4+) with pre-computed column stats
            // ColumnarAggregator handles SUM/COUNT/AVG/MIN/MAX on compressed data
            ...
            // ── FULL SCAN ────────────────────────────────────────────
            // plan: useRowPath=true AND useColumnarPath=true
            ...
            if (plan.type == query::QueryType::FULL_SCAN)
            {
                ...
                if (stmt->whereClause)
                    allRows = applyWhereFilter(allRows, schema,
                                               stmt->whereClause);

                return ResultFormatter::fromScanRows(schema, allRows,
                                                     selectedCols);
            }

            // ── RANGE SCAN ───────────────────────────────────────────
            // plan: router already decided row vs columnar based on
            //       estimated row count vs columnarThreshold (default 10K)
            // Small (<10K): useRowPath=true,  useColumnarPath=false
            // Large (>10K): useRowPath=true,  useColumnarPath=true
            if (plan.type == query::QueryType::RANGE_SCAN)
            {
                ...
                // Apply non-PK WHERE conditions in memory
                if (stmt->whereClause)
                    rangeRows = applyWhereFilter(rangeRows, schema,
                                                 stmt->whereClause);

                return ResultFormatter::fromScanRows(schema, rangeRows,
                                                     selectedCols);
            }
        ...
        // ================================================================
        // DDL — CREATE TABLE
        // Builds TableSchema from AST and hands to SchemaRegistry
        // ================================================================
        ...
        // ================================================================
        // DML — INSERT
        // Always OLTP — goes straight to memtable via engine_.put()
        // Router not involved — writes are always row path
        // ================================================================
        ResultSet executeInsert(const hsql::InsertStatement *stmt)
        {
            ...
            if (!schema.has_value())
                return ResultSet::error("Table '" + tableName +
                                        "' does not exist");
            ...
            for (size_t i = 0; i < colNames.size(); ++i)
            {
                ...
                if (!val.has_value())
                    return ResultSet::error(
                        "Could not evaluate value for column '" +
                        colNames[i] + "'");
                row[colNames[i]] = *val;
            }
        ...
        ResultSet executeUpdate(const hsql::UpdateStatement *stmt)
        {
            ...
            if (!stmt->where)
                return ResultSet::error(
                    "UPDATE without WHERE is not supported");
        ...
        bool compareOp(const hsql::Expr *expr, const Row &row,
        ...
        hyriseTypeToColumnar(const hsql::ColumnType &t) const
        {
            switch (t.data_type)
            {
