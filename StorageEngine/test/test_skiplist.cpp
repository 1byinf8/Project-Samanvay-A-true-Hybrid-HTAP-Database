...
struct TestValues {
    static std::vector<Entry> values() {
        return {
            Entry("key1", "value1", 1),
            Entry("key5", "value5", 2),
            Entry("key10", "value10", 3),
            Entry("key15", "value15", 4),
            Entry("key20", "value20", 5),
            Entry("key25", "value25", 6),
            Entry("key30", "value30", 7),
            Entry("key42", "value42", 8),
...
void testSequentialInsert() {
    ...
    for (const auto &v : vals) sl.add(v);

    for (const auto &v : vals) assert(sl.search(v) == true);
...
void testRandomInsertDelete() {
    ...
    for (const auto &v : subset) sl.add(v);
...
void testStress() {
    ...
    for (const auto &v : vals) sl.add(v);
    for (const auto &v : vals) assert(sl.search(v) == true);
    for (const auto &v : vals) assert(sl.erase(v) == true);
    for (const auto &v : vals) assert(sl.search(v) == false);
...
void testHTAPMixedWorkload() {
    ...
    auto mixed_worker = [&](int thread_id) {
        std::random_device rd;
        std::mt19937 gen(rd() + thread_id);
        std::uniform_int_distribution<> op_dis(0, 3); // 0=insert, 1=search, 2=delete, 3=tombstone
        std::uniform_int_distribution<size_t> val_dis(0, values.size() - 1);
...
int main() {
    std::cout << "Starting Skiplist Regression Tests with Thread Safety for Entry\n";
