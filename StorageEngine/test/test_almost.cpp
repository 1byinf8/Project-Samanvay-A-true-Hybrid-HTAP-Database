...
#include "../src/memTable.cpp" // Adjust path as needed
...
private:
    ...
    std::string generateValue(int id) {
        return "value_" + std::to_string(id) + "_data_with_some_content_to_make_it_longer";
    }
    ...
    void printProgress(int current, int total, const std::string& operation) {
        if (current % (total / 100) == 0 || current == total) {
            ...
            std::cout << "\r" << operation << ": " << std::fixed << std::setprecision(1) 
                      << percentage << "% (" << current << "/" << total << ")" << std::flush;
        }
    }
...
public:
    void testBasicOperations() {
        ...
        logTest("Basic Search", search_result.has_value() && search_result.value() == "test_value");
        ...
        logTest("Search After Reinsert", reinsert_search.has_value() && reinsert_search.value() == "new_value");
    }
    ...
    void testConcurrentInserts() {
        ...
        logTest("Concurrent Inserts", 
                successful_inserts.load() == expected,
                std::to_string(successful_inserts.load()) + "/" + std::to_string(expected) + 
    ...
    void testConcurrentReads() {
        ...
        for (int i = 0; i < NUM_KEYS; ++i) {
            manager.insert("read_key_" + std::to_string(i), "value_" + std::to_string(i));
        }
        ...
        for (int t = 0; t < NUM_THREADS; ++t) {
            threads.emplace_back([&, t]() {
                ...
                for (int i = 0; i < OPERATIONS_PER_THREAD; ++i) {
                    ...
                    if (result.has_value() && result.value() == ("value_" + std::to_string(key_id))) {
                        successful_reads++;
                    }
                }
            });
        ...
        logTest("Concurrent Reads", 
                successful_reads.load() == total_reads.load(),
                std::to_string(successful_reads.load()) + "/" + std::to_string(total_reads.load()) + 
                " in " + std::to_string(duration.count()) + "ms");
    }
    ...
    void testConcurrentMixedOperations() {
        ...
        for (int t = 0; t < NUM_THREADS; ++t) {
            threads.emplace_back([&, t]() {
                ...
                std::uniform_int_distribution<> op_dis(0, 2); // 0=insert, 1=read, 2=delete
                std::uniform_int_distribution<> key_dis(0, OPERATIONS_PER_THREAD - 1);
                
                for (int i = 0; i < OPERATIONS_PER_THREAD; ++i) {
                    int op = op_dis(gen);
                    std::string key = "mixed_key_" + std::to_string(t) + "_" + std::to_string(key_dis(gen));
                    ...
                    if (op == 0) { // Insert
                        ...
                        read_ops++;
                        auto result = manager.search(key);
                        successful_ops++; // Count all reads as successful (may or may not find)
                    } else { // Delete
                        delete_ops++;
        ...
        logTest("Mixed Operations", 
                ...
                ", Deletes: " + std::to_string(delete_ops.load()) + 
                " in " + std::to_string(duration.count()) + "ms");
    }
    ...
    void testMemTableFlushing() {
        ...
        logTest("MemTable Flushing", 
                ...
                std::to_string(successful_reads) + "/" + std::to_string(expected_reads) + 
                " reads successful after flush in " + std::to_string(duration.count()) + "ms");
    }
    ...
    void testRangeQueries() {
        ...
        for (int i = 0; i < NUM_RANGE_KEYS; ++i) {
            std::string key = "range_" + std::string(4 - std::to_string(i).length(), '0') + std::to_string(i);
        ...
        logTest("Range Query", 
                results.size() == expected_count,
                std::to_string(results.size()) + "/" + std::to_string(expected_count) + 
    ...
    void stressTest() {
        ...
        memTable::MemtableManager manager(512 * 1024 * 1024); // 512MB for stress test
        ...
        for (int t = 0; t < STRESS_THREADS; ++t) {
            threads.emplace_back([&, t]() {
                ...
                std::uniform_int_distribution<> op_dis(0, 4); // 0-2=insert, 3=read, 4=delete
        ...
        // Performance metrics
        double ops_per_second = (double)STRESS_TEST_SIZE / (duration.count() / 1000.0);
        double memory_usage_mb = (double)manager.getCurrentMemoryUsage() / (1024 * 1024);
        
        logTest("Stress Test Completion", 
                completed_ops.load() == STRESS_TEST_SIZE,
        ...
        std::cout << "  Active tables: " << manager.getActiveTableCount() << std::endl;
        std::cout << "  Frozen tables: " << manager.getFrozenTableCount() << std::endl;
    }
    ...
    void testTombstoneHandling() {
        ...
        logTest("Search Before Delete", search1.has_value() && search1.value() == "value1");
        ...
        logTest("Search After Reinsert", search3.has_value() && search3.value() == "value2");
    }
    ...
    void runAllTests() {
        ...
        std::cout << "  Success Rate: " << std::fixed << std::setprecision(1) 
                  << (double)passed_tests.load() / (passed_tests.load() + failed_tests.load()) * 100.0 << "%" << std::endl;
